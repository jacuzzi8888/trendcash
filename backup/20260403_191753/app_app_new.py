"""
Naija Trend-to-Cash Application
Main Flask application with comprehensive security, caching, and error handling.
"""
...
def create_app() -> Flask:
    ...
    if len(secret_key) < 32:
        raise RuntimeError("NTC_SECRET_KEY must be at least 32 characters")
    ...
    def health_detailed():
        ...
        cache_set("health", test_key, "ok", 10)
        checks["cache"] = cache_get("health", test_key) == "ok"
    ...
    def teardown_request(exception=None):
        if hasattr(g, 'request_start'):
            duration = (datetime.now(timezone.utc) - g.request_start).total_seconds()
            LogContext.bind(duration_ms=round(duration * 1000, 2))
        LogContext.clear()
        ...
        with get_db() as conn:
            if request.method == "POST":
                ...
                if category == "__unlock__":
                    ...
                    conn.commit()
                    SettingsCache.delete("category_locked")
    ...
    def trends():
        with get_db() as conn:
            if request.method == "POST":
                ...
                if not topic or not category or not source:
                    ...
                    conn.execute(
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
                        (topic, category, source, velocity, advertiser, commercial, evergreen, utc_now()),
                    )
                    conn.commit()
            ...
            per_page = min(MAX_PAGE_SIZE, int(request.args.get("per_page", DEFAULT_PAGE_SIZE)))
            
            if category_filter:
                category_filter = sanitize_input(category_filter, max_length=50)
                base_query = """SELECT tc.*, 
            ...
            paginated = paginate_query(conn, base_query, params, page, per_page)
    ...
    def trend_discovery():
        from .trends_api import fetch_all_trends, get_interest_over_time, get_related_queries
        from .trends_api import get_related_topics, get_suggestions
        from .source_fetcher import fetch_sources
        from .tasks import ExternalAPICaller
        
        api_caller = ExternalAPICaller(max_retries=3, retry_delay=1.0)
        ...
        with get_db() as conn:
            if request.method == "POST":
                try:
                    ...
                    category = sanitize_input(
                        request.form.get("category", "general").strip() or "general", max_length=50
                    )
                    custom_category = sanitize_input(
                        request.form.get("custom_category", "").strip(), max_length=50
                    )
                ...
                result = api_caller.call(fetch_all_trends, geo if geo else None, category)
                    ...
                    for trend in result["result"]["trends"]:
                        ...
                        cursor = conn.execute(
                            ...
                            (topic, category, source, velocity_score, advertiser_safety_score,
                             commercial_intent_score, evergreen_score, created_at)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
                            (topic, category, source, scores["velocity"], scores["advertiser_safety"],
                             scores["commercial_intent"], scores["evergreen"], utc_now()),
                        )
                        candidate_id = cursor.lastrowid
                        added += 1
                        
                        if auto_fetch and candidate_id:
                            src_result = api_caller.call(
                                fetch_sources,
                                topic=topic,
                            ...
                            if src_result["success"] and src_result["result"].get("sources"):
                                for src in src_result["result"]["sources"]:
                                ...
                                sources_added += len(src_result["result"]["sources"])
            ...
            if keyword:
                interest_result = api_caller.call(get_interest_over_time, [keyword], explore_geo)
                if interest_result["success"] and interest_result["result"]:
                    interest_data = interest_result["result"]
                
                queries_result = api_caller.call(get_related_queries, keyword, explore_geo)
                if queries_result["success"]:
                    related_queries = {
                        "rising": queries_result["result"]["rising"],
                        "top": queries_result["result"]["top"]
                    }
                
                topics_result = api_caller.call(get_related_topics, keyword, explore_geo)
                if topics_result["success"]:
                    related_topics = {
                        "rising": topics_result["result"]["rising"],
                        "top": topics_result["result"]["top"]
                    }
                
                suggestions_result = api_caller.call(get_suggestions, keyword)
                if suggestions_result["success"]:
                    suggestions = suggestions_result["result"]["suggestions"]
            ...
            custom_categories = []
            predefined = {'general', 'betting', 'crypto', 'finance', 'education', 'politics', 
                         'sports', 'entertainment', 'travel', 'jobs', 'test', 'trending'}
            for row in recent_trends:
                try:
                    cat = row.get("category", "") if hasattr(row, 'get') else (
    ...
    def sources(candidate_id):
        with get_db() as conn:
            ...
            if request.method == "POST":
                try:
                    url_val = sanitize_input(request.form.get("url", "").strip(), max_length=2048)
                ...
                if not url_val or not publisher:
                    ...
                    conn.execute(
                        ...
                        (candidate_id, url, publisher, published_at, notes, created_at)
                        VALUES (?, ?, ?, ?, ?, ?)""",
                        (candidate_id, url_val, publisher, published_at, notes, utc_now()),
                    )
                    conn.commit()
    ...
    def _generate_draft(candidate: dict, sources_rows: list, image_policy: str) -> dict:
    ...
    def drafts():
        with get_db() as conn:
            ...
            if status:
                base_query = """SELECT d.*, t.topic FROM drafts d 
    ...
    def create_draft(candidate_id):
        with get_db() as conn:
            ...
            draft = _generate_draft(dict(candidate), [dict(s) for s in sources_rows], image_policy)
            now = utc_now()
            ...
            conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
                (candidate_id, draft["title"], draft["content"], "draft", 
                 draft["last_updated"], now, now, draft["image_policy"]),
            )
            conn.commit()
        ...
        with get_db() as conn:
            ...
            if request.method == "POST":
                ...
                conn.execute(
                    ...
                    WHERE id = ?""",
                    (title, content, last_updated, utc_now(), image_policy, image_prompt, new_status, draft_id),
                )
                conn.commit()
        ...
        with get_db() as conn:
            if request.method == "POST":
                ...
                conn.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)""",
                    (draft_id, int(checks["source_valid"]), int(checks["unique_value"]),
                     int(checks["advertiser_safety"]), int(checks["actionability"]),
                     reviewer, utc_now()),
                )
                conn.execute(
            ...
            paginated = paginate_query(conn, base_query, (), page, DEFAULT_PAGE_SIZE)
    ...
    @login_required
    def publish():
        from .publisher import publish_to_site, log_publish
        ...
        with get_db() as conn:
            ...
            history_query = """SELECT pl.*, s.name as site_name, d.title as draft_title 
                FROM publish_log pl 
                JOIN sites s ON pl.site_id = s.id 
                JOIN drafts d ON pl.draft_id = d.id 
                ORDER BY pl.created_at DESC"""
            paginated_history = paginate_query(conn, history_query, (), page, DEFAULT_PAGE_SIZE)
    ...
    def settings():
        with get_db() as conn:
            if request.method == "POST":
                try:
                    limit_val = int(sanitize_input(request.form.get("publish_daily_limit", "20"), max_length=10))
                    ...
                    sources_per = max(1, min(sources_per, 10))
                except ValueError:
                    flash("Numeric values must be valid integers.", "error")
                    return redirect(url_for("settings"))
                
                set_setting(conn, "publish_daily_limit", str(limit_val))
                set_setting(conn, "image_policy_default", 
                           sanitize_input(request.form.get("image_policy_default", "none"), max_length=20))
            ...
            settings_map = {
                "publish_daily_limit": get_setting(conn, "publish_daily_limit", "20"),
    ...
    def metrics():
        with get_db() as conn:
            if request.method == "POST":
                ...
                queries_val = sanitize_input(request.form.get("queries", "").strip() or None, max_length=20)
                ...
                conn.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)""",
                    (metric_date, indexing_rate, queries_val, ctr, avg_position, notes),
                )
                conn.commit()
            ...
            page = max(1, int(request.args.get("page", 1)))
            base_query = "SELECT * FROM metrics ORDER BY metric_date DESC"
            paginated = paginate_query(conn, base_query, (), page, DEFAULT_PAGE_SIZE)
        
        return render_template(
            "metrics.html",
            title=APP_TITLE,
            metrics=paginated["items"],
            pagination=paginated,
        )
    ...
    def ai_generate(candidate_id):
        ...
        api_caller = ExternalAPICaller(max_retries=2)
    ...
    def ai_improve():
        ...
        result = api_caller.call(improve_content, content, instructions, api_key=api_key)
    ...
    def ai_headlines():
        ...
        result = api_caller.call(generate_headline, topic, angle, api_key=api_key)
    ...
    def ai_excerpt():
        ...
        result = api_caller.call(generate_excerpt, content, max_length, api_key=api_key)
    ...
    def ai_faqs():
        ...
        result = api_caller.call(generate_faqs, topic, context, api_key=api_key)
    ...
    def ai_test():
        ...
        api_caller = ExternalAPICaller(max_retries=1)
        
        with get_db() as conn:
            encrypted_key = get_setting(conn, "gemini_api_key", "")
            api_key = decrypt_value(encrypted_key) if encrypted_key else None
        
        result = api_caller.call(test_connection, api_key=api_key)
